...
# Tx Success: 8023:8033: 100% -> 0%
#
# Newer way:
sims <- c(8000, 8001:8010, 8011:8015, 8018:8022, 8016:8017, 8024:8033)
...
for (i in seq_along(sims)) {
  ...
  #vec.eptctinfectundiaghiv <- unname(colMeans(sim$epi$eptctinfectundiaghiv, na.rm = TRUE))
  vec.eptctinfectundiaghiv <- unname(colMeans(sim$epi$eptctinfectundiaghiv / sim$epi$eptpartuptake_ct, na.rm = TRUE))
  #vec.eptctinfectundiaghiv <- unname(colSums(sim$epi$eptctinfecthiv, na.rm = TRUE))
...
#
#
# Reduced Provision (at 20% coverage): 8035, 8045, 8055,8065 (10%->40%)
# Main Only Provision (varying EPT Coverage): 8074:8083 (10--> 100%)
# Increase Main Provision (20% EPT Coverage): 8084:8089 (0.5-->1.0)
# Cas Only Provision (varying EPT Coverage): 8090:8099 (10--> 100%)
# Increase Cas Provision (20% EPT Coverage): 8100:8105 (0.5-->1.0)
# Inst Only Provision (varying EPT Coverage): 8106:8115 (10--> 100%)
