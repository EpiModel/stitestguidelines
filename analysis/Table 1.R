...
# : Annual = 0.1 - 1.0 by 0.1, 364 days, HR = 0%, 182 days
# Varying Higher-Risk Coverage
# 3045, 3087, 3129, 3171, 3213, 3255, 3297, 3339, 3381, 3423
...
# df <- data.frame(anncov, hrcov, annint, hrint, hiv.incid.low, hiv.incid, hiv.incid.high, hiv.hr.low, hiv.hr, hiv.hr.high,
...
#   vec <- num / denom
#   df$asympt.tx[i] <- round(quantile(vec, c(0.5, 0.25, 0.75)), 3)
#
#   num <- unname(colSums(sim$epi$num.rect.tx))
#   denom <- unname(colSums(sim$epi$num.rect.cases))
#   vec <- num / denom
#   df$rect.tx[i] <- round(quantile(vec, c(0.5, qnt.low, qnt.high)), 3)
...
#   denom <- unname(colSums(sim$epi$num.rect.cases))
#   vec <- num / denom
#   round(quantile(vec, c(0.5, qnt.low, qnt.high)), 3)
#
#
#   cat("*")
#
